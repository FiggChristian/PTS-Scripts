    ...
    if (node.nodeType == Node.TEXT_NODE) {
        ...
        while (true) {
            ...
            if (firstMatch == ipAddressMatch) {
                ...
                ipSpan.innerHTML = `<input value="${formatted}" readonly/><span><span>${ipAddressMatch[2]}</span><ul class="${CSS_PREFIX}-smart-text-popup dropdown-menu"><li>IP Address</li><li><button class="btn ${CSS_PREFIX}-smart-text-copy">Copy</button></li><li>Search in:</li><li><ul><li><a target="_blank" href="https://${NETDB_HOST}/fs_node_result?display_order.hardware_address=1&display_order.object=2&display_order.ip_address=3&display_order.node_state=4&display_order.make_and_model=5&display_order.os=6&display_order.department=7&display_order.user=8&column=Name&direction=ascending&purge=&ip_address=${formatted}">NetDB</a></li><li><a target="_blank" href="https://archer.stanford.edu/webacs/welcomeAction.do#pageId=full_search_pageId&query=${encodeURIComponent(formatted)}&forceLoad=true">Cisco Prime</a></li><li><a target="_blank" href="https://iprequest.stanford.edu/iprequest/process/search.jsp?ipaddr=${encodeURIComponent(formatted)}&macaddress=&search=0&sort=UPPER%28lastname%29%2C+UPPER%28firstname%29">IPRequest</a></li></ul></li></ul></span>`;
